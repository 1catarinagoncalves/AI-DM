...
**Status:** Aceito
**Data:** 2026-08-16 · **Revista:** 2026-09-09 (inversão para geração mundo-primeiro) · 2026-09-10 (render em 8 seções + `objective`/`challenges[]`) · 2026-09-14 (exceção pontual em D5 — `summary` volta a consultar o LGMRD, [US-241](../sdlc/01-requisitos/US-241-summary-formula-lgmrd-macguffin.md))
...
> **Exceção pontual (revisão de 14/09, [US-241](../sdlc/01-requisitos/US-241-summary-formula-lgmrd-macguffin.md)).** "Nenhuma tabela do LGMRD entra, nem como espinha nem como tempero" deixa de valer só para o campo `summary`: a sinopse de uma linha volta a nascer de uma rolagem determinística — conceito primário (`1d20quests`) + MacGuffin (`locationsmonumentsanditems`+`conditiondescriptionandorigin` combinadas, OU `patronsandnpcs`) unidos por "because" —, injetada no prompt de autoria como restrição, sem round-trip novo. Motivo: a exclusão total foi decidida olhando o artefato inteiro (o Spike mostrou que autoria livre bate montagem-por-tabela na aventura completa); `summary`, por ser UMA linha só, tem menos espaço pra ganhar especificidade sozinha, e o LGMRD tem fórmula própria testada exatamente pra esse formato. `world`/`story`/`factions[]`/os demais campos autorais continuam sem tabela nenhuma — a exceção é só de `summary`.

...
Hoje **não há login**. O [SetupWizard](../../../apps/web/src/components/setup/SetupWizard.tsx) cria um `User` anônimo com `randomGuestId()` como e-mail e `'Jogador'` como nome, e guarda o `userId` no `localStorage` do navegador (`apps/web/src/lib/session.ts`). Consequências:
...
> **`session.ts` virou code span em 27/07/2026 ([US-79](./US-79-consertar-links-quebrados-na-documentacao.md)).** Era link, e o alvo deixou de existir: o commit `c78d724` — a implementação **desta** story — apagou `apps/web/src/lib/session.ts` ao trocar o `localStorage` pelo token. O parágrafo descreve o estado *anterior* ao login, então a citação continua correta; o que não pode existir é o link. Sem alvo em disco, o `pnpm docs:links` conta como quebrado para sempre. Mesmo tratamento que a US-78 deu ao relatório de eval gitignored.
...
- **Local e produção vivem no mesmo projeto Neon `purple-wave-53471231`, em branches diferentes (corrigido em 2026-09-01).** Até 2026-07-21 eram bancos distintos e este item registrava isso: uma consulta a produção retornava **0 `Character` / 0 `Adventure`**. Depois disso o `DATABASE_URL` local passou a apontar para a branch **`main`** — a mesma do Render —, o que punha `pnpm db:migrate` (`prisma migrate dev`) e os scripts de data migration (`db:migrate:race-class`, `db:migrate:feature-spell`) sobre dado real de jogador. Hoje existe a branch **`dev`** (`br-weathered-hall-aj7qpv25`, criada de `main` em 2026-09-01) e é ela que o `.env` da raiz deve usar; o Render continua na `main`. A consequência para a D1 não muda, só troca de eixo: a reivindicação de órfãos só alcança quem está **na mesma branch** do serviço — personagem criado em `dev` não é reivindicável pelo login de produção, e o critério "absorção da sessão local" só vale quando o anônimo está na branch que o login usa. Para partir de dado igual ao de produção, `reset_from_parent` na `dev`.

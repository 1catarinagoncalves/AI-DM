...
- `start` passa a vir de uma chamada de IA que recebe `registry` + `premissa` + `locations`/`npcs`/`secrets` já gerados — **`hookSeed` NÃO entra nos parâmetros desta chamada**, nem no `prompt`, nem no `system`. `secrets` entra na lista de insumos (não estava no rascunho original desta story) porque o LGMRD trata strong start e secrets/clues como acoplados: "*second only in importance to the strong start*" ([LGMRD.json:234](../../../scripts/lazygm/_data/LGMRD.json), seção `definesecretsandclues`) — um dos lugares onde se planta a primeira pista é a própria abertura. Ver Decisão #1 para a decisão de função dedicada vs. reuso de `ai.generateClosing`.
- O prompt que gera `start` instrui abertura *in medias res* — cena já em ação/tensão, não descrição estática de cenário — espelhando a regra do LGMRD "*a strong start kicks your game off in the middle of the action*" ([LGMRD.json:530](../../../scripts/lazygm/_data/LGMRD.json), seção `strongstarts`) e o fallback "*when in doubt, start with a fight*" ([LGMRD.json:210](../../../scripts/lazygm/_data/LGMRD.json), seção `createastrongstart`): sem conflito óbvio no `registry`/`premissa`/`locations`/`npcs`/`secrets` recebidos, o prompt orienta abrir com confronto ou ameaça imediata.

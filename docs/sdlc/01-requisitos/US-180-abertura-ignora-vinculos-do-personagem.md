...
> **quero** que a abertura (`start`) ancore, quando existir, no local ou NPC que já carrega um vínculo, uma origem ou uma conexão pessoal (`bonds`/`origin.connection`/`origin.memento`) do meu personagem, e que — na ausência de vínculo ou de conflito óbvio — o motor prefira uma cena enraizada no lugar (chegada, gente conhecida, tensão de fundo já pairando) em vez de cair direto em combate,
...
- `buildOpeningBeatPrompt` ganha a LINHA de `complicacao` no `prompt` (não só o tipo em `params`) — mesmo formato que `buildClosingPrompt` já usa para o mesmo campo (`ai.service.ts:260`, `` `Complicação: ${condition} (${description}), origem: ${origin}` ``). Só declarar `complicacao` no `system`/assinatura não basta: sem essa linha no prompt o modelo recebe o TIPO mas nunca o CONTEÚDO, e não tem como decidir entre Enraizada/Confronto por falta do próprio dado.
...
- **Reabrir `setting`/`areaType`** (pilar "ambiente" do artigo) — a US-173 descartou os dois eixos HOJE por não terem consumidor real; esta story não reintroduz esse julgamento. Quem cumpre o papel de "ambiente adequado ao grupo" já não é um eixo de registro escolhido/sorteado à parte: é `locations[]` em si — os ~6 locais que `generateLocationsAndNpcs` (US-158) já gera em prosa a partir do conteúdo bruto rolado (`locais`/`monumentos` do LGMRD, US-147), adequados a nível 1 (único nível que o motor produz hoje, *Ressalva do nível* do backlog do motor) e ao `tone` sorteado. `generateOpeningBeat` já recebe `locations` (não muda nesta story) e é NELE que a cena de abertura ancora — o ambiente do artigo já chega à abertura pela mesma porta usada para o resto.
...
- [x] `generateOpeningBeat` aceita `params.background?: CharacterBackground`, `params.origin?: { connection?: string; memento?: string }` e `params.complicacao: { condition: string; description: string; origin: string }`.
- [x] Quando `background.bonds`/`story`/`flaws` ou `origin.connection`/`origin.memento` tiverem ao menos um item não vazio, o `system`/`prompt` de `generateOpeningBeat` inclui instrução de ancorar a cena no local/NPC ligado a uma dessas âncoras.
- [x] `system` de `generateOpeningBeat` não tem mais um único fallback de combate — oferece **Enraizada** (chegada/gente conhecida/tensão de fundo sem exigir luta) e **Confronto** (ameaça/luta em ação) como estilos nomeados, escolha do modelo pela `premissa`/`complicacao` recebida.
- [x] Confronto continua disponível e é a leitura esperada quando `premissa`/`complicacao` for inerentemente violenta — não é regressão de capacidade, só deixa de ser o único destino. (instrução no `system`; a escolha real do modelo entre os dois estilos fica fora de teste unitário mockado — mesma disciplina da Questão em aberto #1, ver abaixo.)
- [x] `generateAdventure` (`adventure.service.ts`) passa `profile.background`, `profile.origin` e `content.complicacao` a `generateOpeningBeat`.
- [x] `hookSeed` continua fora dos parâmetros de `generateOpeningBeat` — esta story não reabre a disciplina da US-172 (`background`/`origin`/`complicacao` ≠ `hookSeed`).
- [x] **Eval / teste de regressão (estrutural, mesmo padrão da Questão em aberto #1):** (a) fixture com `bonds`/`origin.connection`/`origin.memento` confirma instrução de vínculo no `system`; (b)/(c) fixture com `complicacao` entra no `prompt` (conteúdo real, não só o tipo) e fixture confirma ausência do fallback único antigo + presença dos dois estilos nomeados no `system` — a leitura real do `start` gerado (não-violenta vs. confronto) depende do modelo, fora do escopo do teste unitário mockado; QA manual/`pnpm eval` cobre o comportamento em produção.
...
- `buildOpeningBeatPrompt` ([ai.service.ts:227-246](../../../apps/api/src/ai/ai.service.ts)) precisa da assinatura ganhando `complicacao` E do `return` ganhando a linha de conteúdo — mesma mecânica de `buildClosingPrompt` ([ai.service.ts:248-268](../../../apps/api/src/ai/ai.service.ts)), que já injeta `Complicação: ${condition} (${description}), origem: ${origin}` logo após a linha de `Premissa`. Tipar `complicacao` só em `generateOpeningBeat`/`system` sem tocar `buildOpeningBeatPrompt` é bug silencioso: compila, mas o dado nunca chega no prompt.
...
- **`anchorInstruction` extraído já nesta story, não deixado para limpeza separada.** É a mesma lista de âncoras (`story`/`bonds`/`flaws`/`origin.connection`/`origin.memento`) reusada em `generateSecrets` e `generateOpeningBeat` — mudança mecânica, e deixar duplicado arrisca as duas divergirem com o tempo (favorece nomes greppáveis/funções pequenas, `AGENTS.md`).

---

## Questões em aberto

1. ~~Vale medir com eval dedicado (LLM-judge) se a cena de abertura realmente soa "pessoal" quando há vínculo?~~ **Respondida:** não. O teste estrutural (instrução presente no `system` + nome do NPC/local citado em `start`) basta para esta story — mesmo padrão que US-177/US-179 deixaram em aberto para seus próprios reforços de prompt, sem resolver; aqui a mantenedora decidiu não abrir eval dedicado novo. Se a prosa real em produção mostrar que a instrução não é seguida de forma confiável, eval dedicado vira story própria, não retroage nesta.
...
- [apps/api/src/ai/ai.service.ts:248-268](../../../apps/api/src/ai/ai.service.ts) — `buildClosingPrompt`, molde da linha `Complicação: ...` que `buildOpeningBeatPrompt` precisa espelhar (linha 260) — sem isso `complicacao` fica só no tipo, nunca no conteúdo lido pelo modelo.

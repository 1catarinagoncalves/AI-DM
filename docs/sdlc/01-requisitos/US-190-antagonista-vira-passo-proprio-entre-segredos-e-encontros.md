...
2. ~~`generateOpeningBeat` ecoando o antagonista é sempre desejável, ou às vezes o produto quer a abertura TOTALMENTE cega ao vilão?~~ **Decidido:** sempre insinuar, com a mesma disciplina de dosagem que já protege `conclusion`/`weakness`. Se o eval mostrar abertura revelando demais, é ajuste de prompt, não reversão do insumo. **Revertido em 2026-08-22 por [US-191](./US-191-antagonista-vira-occupant-do-local-do-confronto-final.md):** a proibição de NOMEAR o antagonista (`buildOpeningBeatPrompt`, "NUNCA o nomeie") foi revertida — a abertura passa a poder nomeá-lo. `weakness` continua fora do prompt; só a proibição de nome foi revista.

...
**Status:** ✅ Implementada
**Depende de:** [US-173](./US-173-registro-fica-so-com-tone.md) (✅ implementada em 2026-08-19 — `registry` já é `{ tone }`, sem `setting`/`areaType`, em todo o código)
...
> **Nota (2026-08-19, atualizada após [US-173](./US-173-registro-fica-so-com-tone.md) ✅ implementada):** `registry` já é `{ tone }` (sem `setting`/`areaType`) em todo o código — schema, DTO, prompts do motor, seed e UI. Todo `registry`/`buildDmSystemPrompt`/`streamChat` citado neste documento é só `tone`. A US-173 também deslocou linhas em `adventure.service.ts` (remoção de `setting`/`areaType` acima do trecho que esta story mexe) — referências abaixo já atualizadas pro número de linha pós-US-173.
...
- `createForCharacter` passa `params.mainQuest` e `params.entities: seededEntities` na chamada a `generateOpeningNarration` (linha ~279).
...
- Fallback estático (`openingText = generatedOpening ?? profile.hookSeed`, `adventure.service.ts:301`) quando a chamada de IA falha inteira — esse comportamento é correto e intencional (US-101), não é este bug.
...
- Mudar os prompts do MOTOR (`generateLocationsAndNpcs`/`generateClosing`) que já recebem `tone` — só o Mestre (narração) está cego a ele. `generateSecrets` NÃO recebe `registry` (correção 2026-08-19, ver *Terceiro achado*) — fora do escopo aqui também, mas por motivo diferente (gap não coberto por story nenhuma ainda, não "já recebe corretamente"). **Story própria criada:** [US-176](./US-176-generatesecrets-recebe-tone-do-registro.md).

---

## Critérios de aceite

- [x] `buildOpeningInstruction` aceita `mainQuest?: string | null`.
- [x] Com `mainQuest` presente, a instrução usa esse texto como a fagulha da cena de abertura ("use isto, não `hookSeed`, como o que você dramatiza") — `hookSeed` não é citado em grau nenhum, nem como spark nem como pano de fundo secundário de personagem/classe.
- [x] Sem `mainQuest` (`null`/`undefined`), a instrução cai no comportamento atual (`hookSeed` como spark) — nenhum caminho existente quebra.
- [x] `generateOpeningNarration` (`ai.service.ts`) aceita `entities?: WorldEntity[] | null` e repassa pro `buildTurnStateBlock` da abertura, junto de `mainQuest`.
- [x] `createForCharacter` passa `seededEntities` (já computado antes da chamada) como `entities` — a abertura passa a ver o mesmo elenco que o ledger vai persistir no mesmo turno.
- [x] `tx.adventure.create` grava `generatedAdventure: generated` — coluna deixa de ficar sempre `null`.
- [x] `buildDmSystemPrompt` aceita `tone?` e adiciona uma instrução curta de registo/mood quando presente; ausente (sistema sem motor, ou aventura anterior a esta story) não muda o prompt de hoje.
- [x] A instrução de registo/mood é uma linha genérica (tipo "narre no registo: {tone}") — sem dicionário `tone → instrução` fixo por valor de `TONES`.
- [x] `streamChat` lê `adventure.generatedAdventure` (sem query nova) e repassa `tone` pra `buildDmSystemPrompt` em TODO turno, não só na abertura.
- [x] `createForCharacter`/`generateOpeningNarration` recebem o mesmo campo direto de `generated.tone` (sem esperar o round-trip pelo banco) pra abertura já nascer coerente.
- [x] **Teste de regressão (metade determinística, mesmo molde da US-39):** `buildOpeningInstruction` com `hookSeed` A e `mainQuest` B distintos confirma que o PROMPT carrega B (inclusive `entities` nomeadas) e não contém A — `dm-system.test.ts` (`buildOpeningInstruction — mainQuest domina o gancho fixo`), `ai.service.test.ts` (`AiService.generateOpeningNarration`), `adventure.service.test.ts` (`US-168: seededEntities... chega a generateOpeningNarration`). A metade "a narração de fato HONRA isso" fica pro bake-off da US-17 (mesmo precedente da US-39), não roda aqui.
- [ ] **Eval / teste de regressão:** caso com `tone: 'grimdark'` vs `tone: 'comedic'` (mesmo `mainQuest`) e confirma que a narração de um turno qualquer (não só a abertura) muda de registo entre os dois — a metade determinística (linha de registo entra no `buildDmSystemPrompt`/no system da abertura quando `tone` presente) está coberta em `dm-system.test.ts` e `ai.service.test.ts`; a metade LLM (a narração muda de fato) não foi escrita — decida se entra como caso novo em `evals/cases/` ou fica só no bake-off.
- [ ] `pnpm eval` passa (regra do projeto para qualquer mudança em prompt do DM Agent) — NÃO rodado nesta sessão (custa chamadas reais pagas de LLM); rodar antes de fechar a story.

---

## Notas de implementação

- `GeneratedAdventure.start` **já é** `profile.hookSeed` (`adventure.service.ts:180`, `start: profile.hookSeed` dentro de `generateAdventure`) — e `mainQuest = \`${generated.summary}\n${generated.start}\`` (`adventure.service.ts` em `createForCharacter`). Ou seja, `mainQuest` já contém o `hookSeed` embutido no fim; citar só `mainQuest` na instrução não perde a âncora de continuidade, só corrige o peso (a aventura gerada vem primeiro/dominante, o gancho de classe vira o fecho/tempero).
...
- Terceiro arquivo: [adventure.service.ts](../../../apps/api/src/adventure/adventure.service.ts) → `createForCharacter` (chamada a `generateOpeningNarration`, ~linha 279, passa `mainQuest`/`entities`/`tone`; `tx.adventure.create`, ~linha 320, ganha `generatedAdventure: generated`).
...
- Cast direto (`(adventure.generatedAdventure as GeneratedAdventure | null)?.tone`), não `GeneratedAdventureSchema.parse`, pra extrair `tone` do JSON em `streamChat` — decisão 2026-08-19, ver Questão em aberto #3.
- Com `mainQuest` presente, `hookSeed` some da instrução por completo (decisão da mantenedora, 2026-08-19) — não vira spark nem pano de fundo secundário. A âncora de continuidade não se perde porque `mainQuest` já embutia `hookSeed` no fim (ver nota acima) até a [US-172](./US-172-abertura-gerada-nao-copia-gancho-fixo.md); depois dela `mainQuest` não carrega mais traço nenhum do gancho de classe, o que só reforça a resolução.
- Instrução de registo/mood em `buildDmSystemPrompt` é linha genérica (`"narre no registo: {tone}"`, modelo interpreta o rótulo do catálogo) — decisão da mantenedora, 2026-08-19. Sem dicionário `tone → instrução` fixo: evita ter que sincronizar toda vez que `TONES` (motor) ganhar valor novo.
...
2. ~~A instrução de registo/mood em `buildDmSystemPrompt` deve ser uma frase fixa por `tone` (dicionário `tone → instrução`, precisa cobrir toda a lista de `TONES` do motor) ou uma linha genérica tipo "narre no registo: {tone}" deixando o modelo interpretar a palavra-chave sozinho?~~ **RESOLVIDO (2026-08-19), decisão da mantenedora: linha genérica.** Sem dicionário `tone → instrução` pra sincronizar toda vez que `TONES` ganhar entrada nova; modelo interpreta o rótulo cru do catálogo sozinho.
3. ~~`GeneratedAdventureSchema.parse()` ou cast direto pra extrair `tone` de `adventure.generatedAdventure` em `streamChat` (roda todo turno)?~~ **RESOLVIDO (2026-08-19), decisão da mantenedora: cast direto.** A nota de implementação original citava `SystemConfigSchema.parse(configForLocale(...))` como "mesmo padrão" — checado contra o código, falso: esse `.parse` só existe em `createForCharacter`/`character.service.ts` (caminho de criação, roda 1x). O `streamChat` (`ai.service.ts:453`), hot path que esta US mexe, já lê `config` por cast simples (`configForLocale(...) as Partial<SystemConfig> | null`), sem `.parse`, todo turno — o vizinho real do código, não o exemplo do doc. `tone` segue esse padrão: `(adventure.generatedAdventure as GeneratedAdventure | null)?.tone`. Gate da US-150 já validou o schema inteiro (8 campos aninhados: npcs/secrets/locations/encounters) na escrita — `.parse` por turno seria validação redundante só pra ler 1 string.
...
- [apps/api/src/adventure/adventure.service.ts:180](../../../apps/api/src/adventure/adventure.service.ts) — `start: profile.hookSeed` dentro de `generateAdventure` (prova que `mainQuest` já contém o `hookSeed`).
- [apps/api/src/adventure/adventure.service.ts:301](../../../apps/api/src/adventure/adventure.service.ts) — fallback estático quando `generateOpeningNarration` devolve `null` (comportamento correto, não é este bug).
- [apps/api/src/adventure/adventure.service.ts:267](../../../apps/api/src/adventure/adventure.service.ts) — `seededEntities = seedLedgerFromGeneratedAdventure(generated)`, computado mas nunca repassado à abertura.
...
- [US-173](./US-173-registro-fica-so-com-tone.md) — ✅ implementada; reduziu `registry` a `{ tone }` em todo o código antes desta story rodar (deslocou as linhas de `adventure.service.ts` citadas acima).

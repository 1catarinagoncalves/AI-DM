...
**Depende de:** [US-181](./US-181-antagonista-ganha-want-e-method-estruturados.md) (`antagonist.want`/`antagonist.method` — decidido implementar antes desta: `objective` deve citar motivo/método do antagonista desde a primeira versão, não como retrabalho depois. Ver *Notas de implementação*) · [US-190](./US-190-antagonista-vira-passo-proprio-entre-segredos-e-encontros.md) (**muda COMO esta story recebe `antagonist`** — deixa de ser coproduzido na MESMA chamada de `generateClosing` e passa a chegar pronto, de `generateAntagonist`, chamada anterior; `objective` continua sintetizado em `generateClosing`, só que lendo `antagonist` do parâmetro em vez de gerá-lo junto. Sequenciar depois de US-190 evita implementar a versão antiga da integração pra depois reescrever)

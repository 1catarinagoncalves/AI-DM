...
- **Read-only.** Sem interação nova — nada de "rolar 1d10" nem seleção de linha (ver Fora do escopo).
...
- **Rolar o d10 de verdade** (RNG, botão "rolar") — só exibição do texto/tabela; escolher manualmente ou rolar fisicamente continua sendo o jogador decidindo fora da tela. Extensão natural, não pedida aqui.
- **Copiar a linha escolhida para `Character.background.bonds`/`story`** — US-121 já decidiu que catálogo e prosa livre **não se alimentam automaticamente**; o jogador lê e escreve à mão se quiser aproveitar.

...
- **O catálogo de valores possíveis para `tone`/`setting`/`areaType`** e a validação server-side de chave — isso é [GEN-13](./US-156-catalogos-registro-dto-validacao.md); esta story consome o catálogo quando ele existir, mas roda mesmo sem ele (sorteando entre valores fixos temporários, a decidir na implementação, até GEN-13 chegar).
- **A chamada ao modelo dos segredos** (GEN-6) e a prosa das locações — esta story só produz a lista de escolhas roladas, não texto.
- **Statblocks e orçamento de encontro** — GEN-9, que lê o mesmo `LGMRD.json` mas por outra tabela (`5e_Monster_Builder.json`).
- **Persistir o resultado da rolagem** — o output desta story alimenta GEN-6/GEN-7 em memória, dentro da mesma execução do motor; persistência do artefato final é escopo de GEN-1/US-143.

---

## Modelo de dados proposto

> Sem schema Zod novo formal — o output é a matéria-prima interna do motor, consumida por GEN-6. Se a implementação achar útil, tipar como um objeto intermediário (`RolledAdventureContent`) em `@ai-dm/shared`, mas sem obrigação de persistência.

| Campo | Tipo | Descrição |
|---|---|---|
| `setting` | string (chave) | Rolado uma vez, ou vindo do DTO (GEN-13). |
| `tone` | string (chave) | Idem. |
| `areaType` | string (chave) | Idem. |
| `premissa`, `locais`, `monumentos`, `complicacao` | conteúdo bruto das tabelas do LGMRD | Rolado por peça, pelo mesmo seed — a lista que GEN-6 e a prosa das locações consomem. |
...
- [ ] Cada um dos três campos de registro aceita valor vindo de fora (quando GEN-13 existir) ou sorteia independentemente quando ausente — sem exigir que os três venham juntos.
...
- **Onde este código mora:** junto do `createSeededRandom` (US-146) e do sorteio de statblock (GEN-9) — provavelmente um módulo `adventure-generation` dentro de `apps/api` (o backlog é explícito: "roda no Game Server").
...
2. Enquanto GEN-13 (US-156) não existe, os valores possíveis de `setting`/`tone`/`areaType` para sortear precisam vir de algum lugar — uma lista provisória hardcoded nesta story, substituída quando GEN-13 chegar? Ou esta story espera GEN-13 mesmo não estando formalmente dependente dela (o backlog marca "consome, quando existir")? Decidir na implementação; a lista provisória, se usada, deve ser trivialmente substituível.

---

## Referências no código

- [Backlog — Motor de geração de aventuras one-shot §GEN-4](./backlog-motor-de-geracao-de-aventuras.md) — texto de origem, incluindo *Ordem de geração* (passo 0: registro) e *O desenho: três camadas*.

...
- **Consumir `challenge` na geração real** — nenhum orquestrador (US-164) ou `createForCharacter` lê esse campo do DTO ainda; esta story só garante que o valor **chega** ao backend, não que ele **é usado**. Wiring fica pra quando o orquestrador ganhar esse parâmetro — mesma fronteira que a própria US-161 já deixou aberta pro composer.
...
2. Quando o orquestrador (US-164) ganha `challenge` como parâmetro real — story própria, não estimada aqui.

...
- **Substitui a fonte em `createForCharacter`** ([adventure.service.ts](../../../apps/api/src/adventure/adventure.service.ts)): quando a aventura vem do motor ([GEN-10](./US-153-aventura-deixa-de-ser-derivada-da-classe.md) troca o caminho), `seedLedgerFromGeneratedAdventure` substitui `extractOpeningEntities` como fonte de `tx.adventure.create({ data: { entities } })`. O `map()` em si é o mesmo padrão — só a fonte muda de "extração por LLM" para "leitura do artefato validado".
...
- **Locais e encontros como `WorldEntity`** — o backlog especifica só segredos e NPCs; locais podem entrar em story futura se a evidência pedir (não listado no escopo do GEN-8 original).
...
- [ ] Quando a aventura vem do motor gerado, `tx.adventure.create({ data: { entities } })` recebe o resultado desta função — não mais o de `extractOpeningEntities` (essa troca é condicionada pela [GEN-10](./US-153-aventura-deixa-de-ser-derivada-da-classe.md), que decide quando "a aventura vem do motor").
...
- [Backlog — Motor de geração de aventuras one-shot §GEN-8](./backlog-motor-de-geracao-de-aventuras.md) — texto de origem.

...
- **Mudar `US-125`** — não é necessário: a US-125 já foi ajustada para excluir `'feature'` do seu `EXCLUDED_BENEFIT_TYPES` desde a sua própria criação (mesmo tratamento que já dá a `ability_score`/`skill_proficiency`, mecanizados por outras stories), então não há duplicação no prompt nem dependência de qual das duas stories é implementada primeiro.
...
- **`US-125` já exclui `'feature'`** em `EXCLUDED_BENEFIT_TYPES` (`packages/ai-engine/src/prompts/dm-system.ts`, quando ela for implementada) — nenhuma coordenação de ordem é necessária entre as duas stories; implementar esta antes, depois, ou em paralelo da US-125 dá o mesmo resultado (sem duplicação no prompt).
...
- [US-125](./US-125-beneficios-origem-no-system-prompt.md) — `EXCLUDED_BENEFIT_TYPES` já exclui `'feature'` desde a criação dela; nenhuma coordenação de ordem entre as duas.

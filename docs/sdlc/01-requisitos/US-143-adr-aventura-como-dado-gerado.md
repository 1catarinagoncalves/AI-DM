...
**Status:** ✅ Implementada
**Depende de:** nenhuma
**Relacionado:** [Backlog — Motor de geração de aventuras one-shot](./backlog-motor-de-geracao-de-aventuras.md) (US-143, caminho crítico) · [ADR 012](../../adr/012-aventura-gerada-como-dado.md) (documento produzido por esta story) · [ADR 003](../../adr/003-sistemas-como-dado.md) (sistema como dado — mesmo raciocínio aplicado a aventura) · [backlog das aventuras autorais](./backlog-aventuras-autorais-lazygm.md) (decisão aberta 1, herdada e tornada urgente por esta ADR)
...
> **para que** a [US-144](./US-144-schema-aventura-shared.md) (schema em `@ai-dm/shared`) tenha uma resposta em vez de um campo desenhado no escuro.
...
O [backlog do motor](./backlog-motor-de-geracao-de-aventuras.md) inverteu a ordem das duas frentes de geração de aventura: o motor passa a rodar **antes** de qualquer aventura escrita à mão existir, e o schema que ele emite ([US-144](./US-144-schema-aventura-shared.md)) deixa de ter um exemplar de referência (AV-1) para copiar. Isso reabre, sob pressão de calendário, a *decisão aberta 1* do [backlog irmão](./backlog-aventuras-autorais-lazygm.md): **a aventura é dado de um sistema (regenerável) ou entidade reusável (persistida)?** Sem resposta, a US-144 não sabe se está desenhando um DTO efêmero ou uma linha de banco.
...
Uma ADR curta, decidida **antes** da US-144 escrever uma linha de Zod: grava-se **os dois** — o artefato gerado inteiro (fonte que a [US-151](./US-151-semear-ledger-segredos-gerados.md) semeia no ledger e que a eval pina) e o `seed` que o regenera (o que a US-49/eval usa para reprodutibilidade). Decide também **onde** o artefato mora: reusar `Adventure.entities` como a AV-1 propunha, ou abrir coluna própria (`Adventure.generatedAdventure`, por exemplo) para não colidir com o ledger de `WorldEntity[]` que já vive ali.
...
- **Decisão 1 — regenerável ou congelada:** recomendação do backlog é gravar os dois: o artefato (JSON) porque a [US-151](./US-151-semear-ledger-segredos-gerados.md) semeia o ledger a partir dele e a [US-154](./US-154-eval-aventura-gerada.md)/eval precisa inspecionar o que foi gerado sem rerodar o motor; o `seed` porque é o que a eval pina e o que torna "a mesma ficha regenera a mesma aventura" verificável sem persistir nada.
...
- **Onde o `seed` mora:** campo em `Adventure` (ex. `Adventure.order` já existe e compõe o seed com `Character.id`, ver [US-146](./US-146-seed-deterministico-motor-aventura.md) — decidir se o seed em si precisa de coluna própria ou é sempre recomputável de `characterId + order`).
- A ADR **bloqueia** a [US-144](./US-144-schema-aventura-shared.md): a forma do campo `id`/persistência do schema Zod depende desta decisão.

### Fora do escopo

- **O schema Zod em si** (`levelRange`, `npcs[]`, `secrets[]`, etc.) — é a [US-144](./US-144-schema-aventura-shared.md), que consome a decisão desta ADR mas não a escreve.
- **Migração Prisma.** Se a decisão for coluna nova, a migração em si roda dentro da [US-144](./US-144-schema-aventura-shared.md) (que já muda `@ai-dm/shared`) ou de uma story de infraestrutura mínima — esta US só decide o nome/formato, não aplica `prisma migrate`.
...
- [x] A [US-144](./US-144-schema-aventura-shared.md) consegue começar sem reabrir esta decisão — critério de saída informal, verificado pela story seguinte não ter uma "Questão em aberto" duplicando esta.
...
- [Backlog — Motor de geração de aventuras one-shot §Tarefas (GEN-0)](./backlog-motor-de-geracao-de-aventuras.md) (US-143) — texto de origem desta story.

...
2. ~~**"Entidades do mundo" citado incondicionalmente, emitido condicionalmente.**~~ **Fechada em 2026-07-27: fica fora desta story, e o conserto óbvio está proibido.** `entitiesSection` é `''` quando o ledger está vazio (`:429`), mas `:314` e `:349` afirmam sem ressalva que ele é a fonte de verdade — o prompt aponta para um bloco ausente. E ledger vazio **não é falha**: `extractOpeningEntities` é best-effort por design (`adventure.service.ts:127-135` — falha/vazio → ledger vazio) e `recordEntity` é discricionário do modelo, então uma campanha nova roda vários turnos sem entidade nenhuma. Por que fecha sem virar trabalho:
   ...
   **Virou story própria: [US-87](./US-87-bloco-de-entidades-ausente-citado-no-prompt.md), implementada em 2026-07-29.** Exatamente o conserto no emissor previsto acima — cabeçalho incondicional, corpo vazio apontando para `recordEntity`, camada 2 intacta (custo de cache zero). O eval de gatilho foi **descartado**, não executado: o custo da medição excedia o de um diff cuja reversão é um `git revert`, e a própria previsão registrada aqui ("o modelo simplesmente ignora") tornava o experimento incapaz de decidir. A US-87 também corrigiu a premissa de frequência desta questão — ledger vazio não é o estado normal de campanha nova, é a via em que `extractOpeningEntities` falha.

...
**Relacionado:** [US-41](./US-41-features-traits-de-classe.md) (`FeaturesPanel` — precedente de card "nome + descrição" reaproveitado aqui) · [US-124](./US-124-exibir-beneficios-narrativos-origem.md) (mesma exibição, mas na TELA DE CRIAÇÃO; os dois tipos que exigem parser Markdown ficam fora desta story também, ver §Fora do escopo) · [US-125](./US-125-beneficios-origem-no-system-prompt.md) (mesmo filtro de tipos de benefício, mas para o PROMPT do mestre — os dois consomem o mesmo dado sem se tocar)
...
- **`adventures_and_advancement`/`connection_and_memento`** — mesma exclusão da US-125: texto longo/Markdown com tabela d10 exige o parser que só a US-124 justifica escrever; essa story mostra isso na CRIAÇÃO, não na ficha em jogo. Levar o mesmo parser pra cá é extensão natural, não pedida aqui.

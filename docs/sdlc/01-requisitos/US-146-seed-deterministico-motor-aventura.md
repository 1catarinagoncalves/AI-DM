...
- **O consumo do seed** — qual tabela rola em qual ordem é [US-147](./US-147-rolagem-registro-conteudo.md); a seleção de statblock é [US-152](./US-152-statblocks-papel-orcamento.md). Esta story só entrega a função geradora.
- **Persistir o seed no banco.** Decidido pela [US-143](./US-143-adr-aventura-como-dado-gerado.md) ([ADR 012](../../adr/012-aventura-gerada-como-dado.md)) — se a recomendação de "gravar os dois" for aceita, a coluna que guarda o seed é escopo daquela decisão, aplicada onde a persistência for implementada ([US-144](./US-144-schema-aventura-shared.md) ou a própria criação da aventura).
- **Reseed em caso de falha do gate** (`seed + 1`, [US-150](./US-150-gate-antes-de-persistir-aventura-gerada.md)) — a mecânica de incrementar o seed e tentar de novo é da story do gate; esta story só garante que `seed + 1` produz uma sequência **diferente e igualmente determinística**.
- **"Aleatório" como opção do jogador** ([US-156](./US-156-catalogos-registro-dto-validacao.md)) — aquele conceito é sobre o jogador escolher não fixar `tone`/`setting`/`areaType`; o sorteio subjacente, quando o jogador não escolhe, ainda passa por este mesmo seed (consequência do backlog: *"aleatório" continua determinístico*).

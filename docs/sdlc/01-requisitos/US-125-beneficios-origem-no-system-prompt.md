...
> **Como** jogador,
> **quero** que o mestre conheça os benefícios da **origem** que escolhi na criação (US-122) — um traço especial, um idioma, um item, uma proficiência com ferramenta —,
...
- **`EXCLUDED_TYPES`** = `['ability_score', 'skill_proficiency', 'adventures_and_advancement', 'connection_and_memento']` — constante nomeada com comentário do PORQUÊ de cada exclusão (mecanizado vs. exibido em detalhe na criação), não um número mágico solto.
...
- **Teste de resolução** (`ai.service.test.ts` ou teste unitário da função pura, conforme onde ela morar): origem com os 4 tipos restantes → todos aparecem; origem inexistente/chave vazia → array vazio, sem lançar.
...
- [ ] **Eval / teste de regressão:** `dm-system.test.ts` cobre origem com os 4 tipos restantes (seção aparece, `EXCLUDED_BENEFIT_TYPES` nunca aparece mesmo se presente no array de entrada) e origem ausente (seção some). `ai.service.test.ts` cobre resolução por `origin.key` válido/inválido/ausente.

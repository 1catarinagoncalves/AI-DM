...
- NAME ONCE, then REFER BACK. A proper name earns its place on the FIRST mention of that person/place/thing; from then on use what a narrator uses — a pronoun, an epithet, a body detail, a role. The same proper name landing in paragraph after paragraph (an antagonist's or an artefact's above all), or twice in one paragraph, reads as machine text; it also burns the name's weight, so it lands flat exactly when the scene needs it to hit. This covers the OPTIONS list too: repeating the name in every option is the same failure.

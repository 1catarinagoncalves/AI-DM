// forked from design-sync lib/bundle.mjs - this repo's synth-entry sweep
// bundles apps/web/src/components/** verbatim (no dist/), which pulls in
// real Next.js client modules (next-auth/react's SessionProvider, jose,
// and this app's own src/lib/api.ts) that read bare `process.env.*` /
// `process.platform` / `process.nextTick` at module top level. In a real
// Next build, webpack/SWC's DefinePlugin replaces every one of those before
// the code ever reaches a browser; esbuild's browser target only defines the
// single literal `process.env.NODE_ENV` we already set, so anything else
// throws `ReferenceError: process is not defined` the instant the IIFE
// evaluates - which happened for all 17 components uniformly, since every
// preview loads the same one shared bundle.
//
// ONLY CHANGE from the bundled lib/bundle.mjs: one extra `banner` entry in
// sharedBuildOptions (marked below) that prepends a real `var process = {...}`
// global before any bundled code runs. This does not touch
// stampHeader/resolveDistEntry/the output contract in any way - it only
// prevents a runtime crash during evaluation. Local-only run, never uploaded
// - see .design-sync/NOTES.md "Re-sync risks" before reusing this fork on a
// sync that DOES upload.

// Relative path straight into the staged .ds-sync install, not the bare
// specifier `esbuild` - a `.design-sync/node_modules` symlink to
// `.ds-sync/node_modules` (the Troubleshooting section's usual fix for a
// fork that needs a bare converter dep) didn't take reliably in this
// environment (git-bash `ln -s` produced a plain directory, no reparse
// point, so Node's bare-specifier resolution wouldn't have found it anyway).
// This is more brittle than the symlink across a real re-sync (breaks if
// .ds-sync/ is ever staged from elsewhere) - see NOTES.md "Re-sync risks".
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    // FORK: esbuild's `define` only accepts literals/entity names, not
    // function expressions - process.nextTick needs to stay callable, so a
    // `process: '{...}'` define (tried first) fails "Invalid define value".
    // banner is raw prepended JS instead: a real top-level `var process`,
    // in scope for every bundled module via normal closure, same net effect
    // as the reactShim plugin but for a bare global instead of an import.
    // Real Next.js modules (next-auth/react, this app's src/lib/api.ts) read
    // process.env.* / .platform / .nextTick at module top level, which a
    // real Next build's DefinePlugin normally neutralizes before the browser
    // ever sees it.
    banner: {
      js: 'var process=process||{env:{},platform:"browser",browser:true,version:"",versions:{},nextTick:function(f){var a=Array.prototype.slice.call(arguments,1);queueMicrotask(function(){f.apply(null,a)})}};',
    },
  };
}

...
function pickCandidate(characterId: string, order: number, field: string, candidates: readonly string[]): string {
  const seed = deriveAdventureSeed(`${characterId}:${field}`, order)
 ...
 * juntos.
 */
export function rollRegistry(characterId: string, order: number, overrides: AdventureRegistryOverrides = {}): AdventureRegistry {
  return {
    setting: overrides.setting ?? pickCandidate(characterId, order, 'setting', SETTINGS),
    tone: overrides.tone ?? pickCandidate(characterId, order, 'tone', TONES),
    areaType: overrides.areaType ?? pickCandidate(characterId, order, 'areaType', AREA_TYPES),
  }
}

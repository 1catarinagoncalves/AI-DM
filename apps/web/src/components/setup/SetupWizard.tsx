...
export function SetupWizard() {
  ...
  useEffect(() => {
    ...
    if (session?.userId) {
      ...
      // Cria utilizador automaticamente em segundo plano
  ...
  const attr = (label: string, key: keyof typeof charData) => (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        className="w-full bg-stone-800 border border-stone-600 rounded px-2 py-1 text-center text-white"
      />
    </div>
  )
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex gap-2 mb-8">
          {(['character', 'campaign'] as Step[]).map((s, i) => (
            <div key={s} className={`flex-1 h-1 rounded-full ${step === s ? 'bg-amber-500' : i < (['character', 'campaign'] as Step[]).indexOf(step) ? 'bg-amber-700' : 'bg-stone-700'}`} />
          ))}
        </div>

        {error && <p className="text-red-400 text-sm mb-4 bg-red-950 border border-red-800 rounded p-3">{error}</p>}

        {step === 'character' && (
          <form onSubmit={handleCharacter} className="space-y-4">
            <h1 className="text-2xl font-bold text-amber-400">Crie o seu Personagem</h1>
            <div className="space-y-3">
              <input required placeholder="Nome do personagem"
                value={charData.name} onChange={e => setCharData(p => ({...p, name: e.target.value}))}
                className="w-full bg-stone-800 border border-stone-600 rounded px-3 py-2 text-white placeholder-stone-500" />
              <div className="grid grid-cols-2 gap-3">
                <input required placeholder="Género (ex: Feminino)"
                  ...
                  className="bg-stone-800 border border-stone-600 rounded px-3 py-2 text-white placeholder-stone-500" />
            ...
            <p className="text-stone-400 text-sm">Atributos (1–20):</p>
            ...
            <button type="submit" disabled={loading || !userId}
              className="w-full bg-amber-600 hover:bg-amber-500 disabled:opacity-50 rounded py-2 font-semibold">
        ...
        {step === 'campaign' && (
          <form onSubmit={handleCampaign} className="space-y-4">
            <h1 className="text-2xl font-bold text-amber-400">Inicie a sua Aventura</h1>
            <div className="space-y-3">
